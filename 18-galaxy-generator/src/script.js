...
parameters.insideColor = new THREE.Color(`hsl(${Math.random() * 360}, 100%, 50%)`);
parameters.outsideColor = new THREE.Color(`hsl(${Math.random() * 360}, 100%, 50%)`);
...
const generateGalaxy = () => {
    ...
    // destroy previous galaxy if it exists
    if (points !== null) {
        geometry.dispose();
        material.dispose();
        scene.remove(points);
